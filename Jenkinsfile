...
pipeline {
    ...
    environment {
        DOCKER_USERNAME = "mohamed_cissse" // username docker
    ...
    stages {
        ...
        stage("Checkout") {
            steps {
                git branch: 'master', url: 'https://github.com/MHD6C/tp5.git'
            }
        }
        ...
        stage("Push image to Docker Hub") {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'MHD', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASSWORD')]){
                    bat """
